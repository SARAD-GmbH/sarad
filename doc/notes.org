...
- software_version
- serial_number
- components (e.g. NO_x sensor, pump)
  - id
  - name
  - measurands ("Result Index" in DACM naming)
    - id
    - name
    - items (e.g. recent sampling, average of the last completed interval,
      minimum, error)
      - id
      - name
      - operator
